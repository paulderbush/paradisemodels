  ...
  if (_bookingDraft) {
    ...
    content.innerHTML = `
      <div class="booking-summary">
        ...
        ${d.requestedPriceOnRequest && d.requestedPriceOnRequest.length ? `<div style="margin-bottom:0.65rem;border-top:1px solid var(--glass-border);padding-top:0.65rem">${d.requestedPriceOnRequest.map(n => `<div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-soft);padding:2px 0"><span>${n}</span><span style="color:var(--text-muted)">Rate on request</span></div>`).join('')}</div>` : ''}
