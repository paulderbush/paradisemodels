...
// Only shown signed-out now (see setVipPaywallCopy) — takes the visitor to
// sign in/create an account first, since the manager granting access
// manually still needs to know which account to unlock.
function handleVipCta() {
  window.location.href = '/account/?next=' + encodeURIComponent('/vip-models/');
}

// No automated checkout yet — VIP access is arranged manually over
// Telegram (crypto payment for now; card payers are pointed to the same
// manager). Signed-out visitors still see the old Sign In button since an
// account is required before the manager can grant access; signed-in
// visitors get the Telegram link instead.
function setVipPaywallCopy(signedIn) {
  document.getElementById('vipPaywallCopy').textContent = signedIn
    ? 'VIP access is currently arranged via cryptocurrency payment — message our manager on Telegram to get started.'
    : 'Sign in, then message our manager on Telegram to arrange VIP access.';
  document.getElementById('vipUnlockBtn').style.display = signedIn ? 'none' : 'block';
  document.getElementById('vipTelegramCta').style.display = signedIn ? 'block' : 'none';
  document.getElementById('vipPaymentNote').textContent = signedIn
    ? 'Paying by card instead? Contact the same manager on Telegram.'
